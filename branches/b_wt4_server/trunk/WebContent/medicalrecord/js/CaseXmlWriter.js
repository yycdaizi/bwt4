...
/**第一页*/
//患者信息
var AAATags = ["AAA01", "AAA02C", "AAA03", "AAA04",
"AAA05C","AAA40","AAA42", "AAA06C", "AAA07", "AAA08C", "AAA09", "AAA10", "AAA11", 
"AAA43","AAA44","AAA45","AAA46","AAA47",
"AAA13C", "AAA33C", "AAA14C",
"AAA48","AAA49","AAA50",
"AAA16C", "AAA36C", "AAA17C", "AAA18C", "AAA19", "AAA20", "AAA21C", "AAA22", "AAA23C", "AAA24", "AAA25", "AAA26C", "AAA27", "AAA28", "AAA29"];
//入院信息
var AABTags = ["AAB01",/*入院日期(时)*/
"AAB02C", "AAB03", "AAB06C"];
//出院信息
var AACTags = ["AAC01",/*出院日期*/
 "AAC02C", "AAC03", "AAC04"];
//转经信息
var AADTags = ["AAD01C"];
...
function CaseXmlWriter(){
    ...
    this.generateCase = function(){
        ...
        this.writer.writeEndElement();
        //ABD 其他诊断集合===========================================
        this.writeDataGrid("ABDS",ABDTags);
		...
		//新生儿表格=======================================================
		this.writeDataGrid("AENS",AENTags);
	...
	this.writeDataGrid = function(id,colTags){
		...
		if(!this.isNotEmptyTable(data,colTags)) return;
	...
	this.writeOperationGrid = function(dataArray){
		...
		for(var i=0;i<data.length;i++){
			...
			if(this.isRowNotNull(rowData,colTags)){
				...
				this.writer.writeStartElement(subId);
				for(var j=0;j<colTags.length;j++){
					if($.isEmptyObject(rowData[colTags[j]])) continue;
					this.writer.writeElementString(colTags[j],rowData[colTags[j]]);
				}
				this.writer.writeEndElement();
			}
	...
	this.writeZZQKGrid = function(){
		...
		if(!this.isNotEmptyTable2(data,AEKTags)) return;
		this.writer.writeStartElement(id);
		for(var i=0;i<data.rows.length;i++){
			var rowData = data.rows[i];
			if(this.isRowNotNull(rowData,AEKTags)){
				var subId=id.substring(0,id.length-1);
				this.writer.writeStartElement(subId);
				for(var j=0;j<AEKTags.length;j++){
					this.writer.writeElementString(AEKTags[j],rowData[AEKTags[j]]);
				}
				this.writer.writeEndElement();
			}
		}
	...
	this.isNotEmptyTable2 = function(data,colTags){
		for(var i=0;i<data.rows.length;i++){
			var rowData = data.rows[i];
			if(this.isRowNotNull(rowData,colTags)){
				return true;
